...
class TrackingFragment : Fragment(R.layout.fragment_tracking) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState != null) {
            val cancelTrackingDialog = parentFragmentManager.findFragmentByTag(CANCEL_DIALOG_TRACKING_TAG) as CancelTrackingDialog?
    ...
    private fun endRunAndSaveToDb() {
        map?.snapshot { bitmap ->
            ...
            val avgSpeed = round((distanceInMeters / 1000.0f) / (currentTimeInMillis / 1000.0f / 60 / 60) * 10) / 10.0f
            val dateTimestamp = Calendar.getInstance().timeInMillis
            val caloriesBurned = ((distanceInMeters / 1000.0f) * weight).toInt()
    ...
    private fun addLatestPolyline() {
        if (pathPoints.isNotEmpty() && pathPoints.last().size > 1) {
            ...
            var lastLastLng = pathPoints.last().last()
